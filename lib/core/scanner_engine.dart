...
class ScannerEngine extends ChangeNotifier {
  ...
  void setTimeframeSeconds(int seconds) {
    if (seconds != 60 && seconds != 300 && seconds != 900) return;
    if (timeframeSeconds == seconds) return;
  ...
  Map<String, AssetState> get assetStates => Map.unmodifiable(_assets);

  AssetState? stateFor(String symbol) => _assets[symbol];
  ...
  Future<void> useFeed(FeedAdapter adapter) async {
    ...
    // FEED ISOLATION: clear previous provider state.
    // Prevent DEMO/OTC candles and signals from surviving
    // when switching to Twelve Data LIVE, and vice versa.
    _assets.clear();
    _results.clear();
    totalTicks = 0;
    lastTickAt = null;
