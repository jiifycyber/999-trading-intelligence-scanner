...
import 'radar_enabled_dashboard.dart';
...
import 'trading_menu_drawer.dart';
...
class _DashboardState extends State<Dashboard> {
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        return Scaffold(
          drawer: TradingMenuDrawer(
            onSignalHistory: () {
              Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (_) => TradingChartScreen(engine: engine),
                ),
              );
            },
          ),
          body: LayoutBuilder(
            builder: (context, constraints) {
              if (constraints.maxWidth >= 900) {
                return RadarEnabledDashboard(
                  engine: engine,
                  results: engine.rankedResults,
                );
              }
